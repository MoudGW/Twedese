...
function getBaseUrl() {
	...
	$port = SiteModule::$siteInfo->port;
	if (!$port && isset($_SERVER['SERVER_PORT']) && intval($_SERVER['SERVER_PORT'])) {
	...
	try {
		...
		if (!$apiPostUrl) {
			...
			if (version_compare(phpversion(), '7.4') >= 0) {
				PHPMailer::$validator = function($address) { return true; };
			}
...
function is_mail($mail) {
	if (preg_match("/^[0-9a-zA-ZА-Яа-я\.\-\_]+\@[0-9a-zA-ZА-Яа-я\.\-\_]+\.[0-9a-zA-ZА-Яа-я\.\-\_]+$/isu", trim($mail)))
		return trim($mail);
	return "";
}
